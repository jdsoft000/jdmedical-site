...
# Logo Colors
bright_blue = (0.1, 0.45, 0.85) # 밝은 청색 (J)
teal = (0, 0.6, 0.55)           # 청록색 (D)
...
for c in valid_contours:
    ...
    # J is around x=96, D is around x=308. We can use 200 as the split point.
    if bx < 200:
...
shape_j.finish(color=bright_blue, fill=bright_blue)
shape_j.commit()

shape_d.finish(color=teal, fill=teal)
shape_d.commit()

def mm_rect(x1, y1, x2, y2):
    return fitz.Rect(x1*s, y1*s, x2*s, y2*s)

# Restored text colors to light cyan
